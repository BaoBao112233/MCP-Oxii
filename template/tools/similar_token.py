...
from template.tools.twitter.twitter import PageContentGetter

absolution_path = os.path.abspath(__file__)
curls_dir = os.path.join(os.path.dirname(absolution_path), 'curls', 'geckoterminal')
...
def get_similar_token_v2(
    base_url: Annotated[str, Field(description="The base URL of the API endpoint")],
    contract_address: Annotated[str, Field(description="The contract address of the token to find similar tokens for")],
    include_market_info: Annotated[bool, Field(default=True, description="Whether to include market information in the response")]
):
    """
    ...
    Args:
        ...
        include_market_info (bool): Include market information
...
def get_similar_token(base_url: str, contract_address: str, include_market_info: bool = True):
    """
    Lấy dữ liệu từ API TokenSuggest và trích xuất address và symbol của 5 đối tượng đầu tiên
    Nếu include_market_info=True, bổ sung thông tin marketcap và liquidity
    
    Args:
        base_url (str): URL cơ sở của API
        contract_address (str): Địa chỉ contract cần truy vấn
        include_market_info (bool): Có lấy thông tin thị trường không
        
    Returns:
        str: Chuỗi chứa address, symbol và thông tin thị trường của các token tương tự
    """
...
def get_market_info_parallel(tokens):
    """
    Lấy thông tin thị trường cho nhiều token song song
    
    Args:
        tokens (list): Danh sách các token cần lấy thông tin
        
    Returns:
        list: Danh sách token đã bổ sung thông tin thị trường
    """
